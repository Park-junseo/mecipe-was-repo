 ...
 */
export function getCommandParameters(command: string, parameters: string[], delimiter: string = ':'): string[][] {
    const result: string[][] = [];
    
    for (const parameter of parameters) {
        if (parameter.startsWith(`${command}:`)) {
            // --seed: 제거
            const seedPart = parameter.substring(`${command}:`.length); // `${command}:`.length = `${command}:`.length
