...
export class MessageCacheService {
  ...
  executeJoinClientEvent(socket: Socket, roomId: string) {
    ...
    this.cacheTypeMap.forEach((cacheType, type) => {
      messagePool.push(
        ...this.getJoinClientEventByTypeMessagePool(roomId, type, cacheType),
      );
    });
  ...
  executeLeaveClientEvent(socket: Socket, roomId: string) {
    ...
    this.cacheTypeMap.forEach((cacheType, type) => {
      messagePool.push(
        ...this.getLeaveClientEventByTypeMessagePool(roomId, type, cacheType),
      );
   ...
   * @returns 메시지 풀
   */
  private getJoinClientEventByTypeMessagePool(
    roomId: string,
    type: string,
    typesCacheType: CacheTypeFlag,
  ) {
    const messagePool = this.getRoomMessagePool(
      roomId,
      type,
      typesCacheType,
   ...
   * @returns 메시지 풀
   */
  private getLeaveClientEventByTypeMessagePool(
    roomId: string,
    type: string,
    typesCacheType: CacheTypeFlag,
  ) {
  ...
  private clearMessageCacheWithCacheType(
    clientId: string,
    roomId: string,
    type: string,
    typesCacheType: CacheTypeFlag,
   ...
   * @returns 메시지 풀풀
   */
  private getRoomMessagePool(
    roomId: string,
    type: string,
    typesCacheType: CacheTypeFlag,
    targetCacheType: CacheTypeFlag,
  ) {
    ...
    return (
      this.roomMessageCache.get(roomId)?.getAllClientMessageCache(type) ?? []
    );
  }
  ...
  private logRoomMessagePoolInfo(roomId: string, messagePoolSize: number) {
    ...
    const cacheTypes = [...this.cacheTypeMap.entries()].filter(
      ([_, cacheType]) =>
        cacheType & (CacheTypeFlag.RECORD_EVERY | CacheTypeFlag.NON_VOLATILE),
    );
    if (cacheTypes.length > 0) {
      this.logger.warn(
        `The Type of Room message pool (${messagePoolSize}) can be very heavy: ${cacheTypes
          .map(([type, _]) => type)
          .join(', ')} ${roomId}`,
      );
    }
